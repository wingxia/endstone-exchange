...
一个面向 Endstone 的 C++ 物品交易插件。管理员用名为 `exchanger` 的木棍把世界中的方块或实体设为交易点，玩家直接右击目标即可查看价格并交易。当前插件版本为 **1.5.0**，锁定 **Endstone v0.11.6 / BDS 1.26.33**。

## 功能

- 名为 `exchanger` 的木棍右击目标：开启交易；再次右击：关闭这个交易点，但保留未完成订单以及暂存的余额和物品。
- 普通右击交易目标：先显示“有人要买”“有人在卖”、余额、可出售数量和可用交易方式。只有存在出售订单时才显示“直接购买”，只有存在收购订单时才显示“直接出售”。
- 选择交易方式后，用输入框填写整数数量；按自己的价格交易时再填写 `1～5000` 的整数价格，直接购买或直接出售时不生成价格输入和价格调整按钮。确认页可用 `-100、-10、-1、+1、+10、+100` 调整数量和价格，也可返回输入框重填。Bedrock 原生协议不能在同一张表单里混排输入框和按钮，因此填写页与调整确认页分开显示，按钮按客户端原生布局排列。
...
- 交易界面会把实际校验条件逐项写清楚，包括自定义名称、说明、附魔名称和等级、耐久损耗、铁砧修复代价以及其他特殊属性；出售失败时复用同一份清单。
