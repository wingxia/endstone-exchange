...
- 2026-08-27 在隔离的 NAS UDP 19141、全新地图 `ExchangeTestFreshV1100`、独立数据库、BDS 1.26.40 与 Endstone 0.11.8 运行时完成真实 Bedrock 假人回归；生产服进程未参与测试。
- 普通、连锁和循环命令方块入口均通过。核心场景中卖家背包有 5 个钻石块与 7 根羽毛，买单以 `5u` 收购 2 个：确认后 2 个成交并获得 `10u`，其余 3 个以 `1u` 挂单，在线背包探测确认钻石块为 0、7 根非交易物羽毛完整保留；买家重连后恰好领取 2 个钻石块。
- 限价买卖、直接买卖、部分成交、价格优先、取消返还、退款、离线领取、重连、共享订单簿、市场关闭/重开、非 OP 权限拒绝和保护范围内外交互均通过。
- 持久化核对确认买单已成交、卖单剩余 3 件且价格为 `1u`、成交为 2 件 × `5u`，资金账本差异与暂存、领取、转账恢复队列均为 0；最终卸载全部临时 E2E 驱动并以正式产物重启，恢复全新地图上的 1 个活跃交易点，数据库正常，未完成/待核对直接转账为 0。
