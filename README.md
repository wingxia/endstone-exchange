...
- `resources/items.yml`：由 Minecraft Wiki 生成的商品分类、名称和图标配置快照。
- `tools/update_items_from_wiki.py`：从 Minecraft Wiki `Blocks2`/`Items2` 拉取 Bedrock 物品和方块目录，并生成 `resources/items.yml` 与 C++ catalog。
...
```bash
cmake -G Ninja -S . -B build-plugin -DEXCHANGE_BUILD_ENDSTONE_PLUGIN=ON -DEXCHANGE_BUILD_TESTS=OFF
...
```bash
python3 tools/update_items_from_wiki.py
```
...
Core service, searchable generated catalog, form UI, inventory-backed sell flow, mailbox claim flow and tests are implemented. The MySQL schema and service interfaces are complete; the current plugin still uses `InMemoryRepository`, so test-server orders persist only while the server process is running. A production MySQL repository can be attached behind the `Repository` interface without changing UI or matching behavior.
