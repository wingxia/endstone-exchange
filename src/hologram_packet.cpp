...
constexpr std::uint32_t EntityDataName = 4;
...
constexpr std::uint32_t EntityDataTypeString = 4;
...
void appendString(std::string &payload, const std::string_view value) {
    appendVarUint(payload, value.size());
...
std::string hologramAppearancePacket(const std::uint64_t runtime_id,
                                     const std::optional<std::string_view> name_tag) {
    std::string payload;
    payload.reserve(32);
    appendVarUint(payload, runtime_id);

    // Entity metadata map.
    appendVarUint(payload, name_tag ? 5 : 4);
    if (name_tag) {
        appendVarUint(payload, EntityDataName);
        appendVarUint(payload, EntityDataTypeString);
        appendString(payload, *name_tag);
    }
