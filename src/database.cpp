...
void Database::migrate() {
    ...
    const auto version_8_applied = query("SELECT version FROM exchange_schema_versions WHERE version=8");
    if (version_8_applied.empty()) {
        execute(R"sql(CREATE TABLE IF NOT EXISTS exchange_economy_transfers (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
            ...
            status ENUM('PREPARED','EXTERNAL_APPLIED','COMPLETED','FAILED','BLOCKED') NOT NULL DEFAULT 'PREPARED',
            external_balance_before_units BIGINT NULL,
        ...
        execute("INSERT INTO exchange_schema_versions(version) VALUES (8)");
    }

    const auto version_9_applied = query("SELECT version FROM exchange_schema_versions WHERE version=9");
    if (version_9_applied.empty()) {
        execute(R"sql(CREATE TABLE IF NOT EXISTS exchange_frame_listings (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
        ...
        execute(R"sql(CREATE TABLE IF NOT EXISTS exchange_frame_listing_bindings (
            target_key VARCHAR(255) NOT NULL PRIMARY KEY,
        ...
        execute("INSERT INTO exchange_schema_versions(version) VALUES (9)");
    }

    const auto version_10_applied = query("SELECT version FROM exchange_schema_versions WHERE version=10");
    if (!version_10_applied.empty()) {
        return;
    }
    const auto before_balance_column = query(R"sql(SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='exchange_economy_transfers'
          AND COLUMN_NAME='external_balance_before_units' LIMIT 1)sql");
    if (before_balance_column.empty()) {
        execute(R"sql(ALTER TABLE exchange_economy_transfers
            ADD COLUMN external_balance_before_units BIGINT NULL AFTER status)sql");
    }
    execute("INSERT INTO exchange_schema_versions(version) VALUES (10)");
}
