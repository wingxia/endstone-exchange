...
| UMoney balance integration | `PluginManager::getPlugin`, pybind11's live Python subclass recovery, `api_get_player_money(player_name)` and `api_change_player_money(player_name, delta)` | Optional direct plugin call on the Endstone thread; no HTTP bridge |
