...
- Label carriers remain normal protected actors on the server, while a small public `Player::sendPacket` metadata update makes them `0.01` scale with a zero client collision box. Block labels rest on the block instead of being teleported every refresh. The loaded-chunk index prevents off-screen spawning, a 20-tick grace period lets persisted actors return before any replacement is created, and chunk reconciliation removes stale duplicates. A reconnecting player triggers one nearby carrier recreation after spawning or crossing into the target chunk, guaranteeing Bedrock receives `AddActor` before the repeated appearance metadata.
